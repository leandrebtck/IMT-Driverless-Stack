#!/bin/bash
# ============================================================
# LAUNCHER - Skidpad — YOLO LiDAR + SLAM LiDAR
# ============================================================
...
RVIZ_CONFIG="$RVIZ_DIR/skidpad_lidar.rviz"
...
if command -v zenity &>/dev/null; then
    zenity --info --title="Skidpad LiDAR" \
...
gnome-terminal --title="CONE MAPPER SKIDPAD LIDAR" -- bash -c "
    $ROS_CMD; python3 $SLAM_DIR/cone_mapper_skidpad_lidar.py; exec bash" &
sleep 2
...
gnome-terminal --title="SKIDPAD DRIVER" -- bash -c "
    $ROS_CMD;
    python3 $CONTROL_DIR/skidpad_driver.py --map /slam_lidar/cone_map --straight 13.0;
    exec bash" &

sleep 1
echo "Skidpad LiDAR lancé — topic: /slam_lidar/cone_map"
