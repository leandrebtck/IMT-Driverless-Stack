...
Différences vs cone_mapper.py :
  - Blocage virage désactivé (le Skidpad est intégralement composé de virages)
  ...
  - Publie sur /slam_skidpad_stereo/cone_map
"""
...
MAP_TOPIC  = '/slam_skidpad_stereo/cone_map'
STAT_TOPIC = '/slam_skidpad_stereo/stats'
MARKER_NS  = 'cone_map_skidpad_stereo'
...
class ConeMapperSkidpadStereoNode(Node):
    ...
    MAX_DEPTH   = CFG['slam']['stereo']['max_depth_straight_m']
    # Blocage virage désactivé pour Skidpad (tout est courbe)
    MIN_SCORE   = 0.0

    def __init__(self):
        super().__init__('cone_mapper_skidpad_stereo')
        ...
        self.get_logger().info(
            f"ConeMapperSkidpadStereo — merge={self.MERGE_DIST}m  "
...
def main(args=None):
    ...
    node = ConeMapperSkidpadStereoNode()
    try:
        rclpy.spin(node)
